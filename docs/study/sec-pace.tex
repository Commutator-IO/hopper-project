...
and compared on every build, every rule stated where a reader can find it.
Two of the reader's rules were found by bugs (Section 6.2), and the calendar
of Figure~\ref{fig:days} went stale because its generator wrote to a path no
build checked. The assistant does not remove the checks; it makes them the
whole of the guarantee. And when a converter costs an afternoon, what remains
expensive are the editorial decisions --- what a bare figure is, whether
three payments are one sale --- which the code cannot take: the cost of an
edition moves toward its rules and its checking, which is where it belongs.
