...
/**
 * The four the article uses, and no more.
 *
 * This script was written for the critical note that preceded the article, and
 * it still draws that note's figures: the works at their own proportions, the
 * compositional schemas, the travels. The note is abandoned, so those are
 * computed and dropped rather than written — deleting the code would lose the
 * drawings, and writing the files would leave nine figures in the directory
 * that nothing includes and nobody checks.
 *
 * Beside this script, where the article reads them from. They were once
 * written into a scratch directory of the session that made them, which meant
 * the figures could not be regenerated by anybody else — and they went stale:
 * the day count in fig-days was a day out for a week before anyone looked.
 */
const USED = ['fig-formats-scatter', 'fig-materials', 'fig-money', 'fig-days'];
for (const k of USED) {
  if (!out[k]) throw new Error(`figures: ${k} is used by the article and was not drawn`);
  writeFileSync(resolve(import.meta.dirname, `${k}.tex`), out[k]);
}
console.log(
  `figures: ${USED.join(' ')}\n` +
    `         ${Object.keys(out).length - USED.length} more drawn and dropped — the abandoned note's\n`,
);
